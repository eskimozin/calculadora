...
import {suggestionOptionsForBits, suggestionsOptionsForReal, suggestionsOptionsForSubs} from "../../data/config.js";
...
const ValueToTimeForm = ({values, onValueChange, clearValues}) => {
  return (<form className={"m-0 d-flex flex-column gap-3"} onSubmit={() => {
      ...
      <div className="mt-1 d-flex flex-wrap justify-content-start align-items-center gap-1">
        {
          [...suggestionsOptionsForReal].map((v, i) => {
            return (
              <button type={"button"} key={i} tabIndex={-1} className={"btn btn-sm btn-dark bg-body-tertiary text-body-secondary"} onClick={() => {
    ...
    <div className="form-group">
      ...
      <div className="mt-1 d-flex flex-wrap justify-content-start align-items-center gap-1">
        {
          [...suggestionOptionsForBits].map((v, i) => {
            return (
              <button type={"button"} key={i} tabIndex={-1} className={"btn btn-sm btn-dark bg-body-tertiary text-body-secondary"} onClick={() => {
